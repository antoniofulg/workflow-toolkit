...
expected: The README, pack guide, notices, and installed skill set identify optional companion sources and their provenance while keeping the product-neutral workflow scope consistent.
...
Covers public provenance, authorship, clean-room adaptation language, the optional-versus-external
...
The current skills-only contract leaves security-lifecycle and other companions outside the WTK
distribution. Confirm the README names their optional upstream sources and that no companion tree
or alias is installed by the WTK skill set.
