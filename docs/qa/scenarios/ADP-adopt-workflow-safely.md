...
Fresh QA retest at `d363b1c` confirmed this scenario's entry point now matches the README's exact
13-skill selection. Both linked bugs passed retest. The scenario returns to `untested` because the
public migration CLI still has no safe way to observe a publication failure and rollback.
