---
name: wtk-discover
description: 'Explore an unshaped idea to a build or stop verdict and a slice-oriented design artifact. Use for discovery; not decided work or implementation.'
license: CC-BY-4.0
metadata:
...
2. **Identifier ceiling: route, table, column, class, glossary term.** Nothing below - method, container token, file path, CSS selector - appears as design content. Evidence citations may name a source file and line; they support a fact and do not prescribe the implementation. If the plan cannot name the method from the route and the class, that is the plan's problem, and a design that names it has started planning and will drift from the plan the moment the plan does its job.
...
Then every cell of the grid lands once: hard-to-reverse decisions in `Key decisions`, reversible defaults in the slice's `Alternatives considered`, and RFC, spike and design items in the slice's status and open question. The Work index shows the grid as one status column. If a reader cannot see the work and what is still fog from that column alone, the index failed.
...
Actions: Do not run this skill. Planning starts from the document in `wtk-plan`; this skill produces the document.
