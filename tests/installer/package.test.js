...
test('IT-010 pack dry-run includes Node installer and bundled security skills', () => { const json = execFileSync('npm', ['pack', '--dry-run', '--json'], { cwd: root, encoding: 'utf8' }); const metadata = JSON.parse(json)[0]; const names = metadata.files.map((item) => item.path); assert.ok(names.includes('bin/wtk.js')); assert.ok(names.includes('scripts/installer/engine.js')); assert.ok(names.includes('docs/toolkit/repository-intelligence.md')); assert.ok(names.includes('.agents/skills/prompt-review/SKILL.md')); for (const skill of ['security-audit-coordinator', 'security-implementation', 'security-review', 'security-spec', 'security-threat-model']) assert.ok(names.includes(`.agents/skills/${skill}/SKILL.md`), skill); for (const name of ['context-handoff.md', 'construction-constraints.md']) assert.ok(names.includes(`.agents/skills/wtk/references/${name}`)); assert.equal(names.some((name) => ['scripts/adopt.py', 'scripts/install_security_skills.py', 'bin/my-workflow.js'].includes(name)), false); });
