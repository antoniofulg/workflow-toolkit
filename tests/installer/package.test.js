...
test('IT-010 pack dry-run includes Node installer and excludes Python adopter', () => { const json = execFileSync('npm', ['pack', '--dry-run', '--json'], { cwd: root, encoding: 'utf8' }); const metadata = JSON.parse(json)[0]; const names = metadata.files.map((item) => item.path); assert.ok(names.includes('bin/wtk.js')); assert.ok(names.includes('scripts/installer/engine.js')); assert.ok(names.includes('docs/toolkit/repository-intelligence.md')); assert.ok(names.includes('.agents/skills/prompt-review/SKILL.md')); for (const name of ['context-handoff.md', 'construction-constraints.md']) assert.ok(names.includes(`.agents/skills/wtk/references/${name}`)); assert.equal(names.some((name) => name === 'scripts/adopt.py' || name === 'bin/my-workflow.js'), false); });
